...
import com.school.book.dao.ShoppingCarDAO;
/**
 * 购物车业务逻辑类
 */
public class ShoppingCarBll {
	...
	public void insertToCar(ShoppingCarBean shoppingCarBean){
		ShoppingCarBean selectCarBean = shoppingCarDAO.selectCarByBookCode(shoppingCarBean);
		//如果原来没有有本本物品就插入，不然就于原来的书本合并
		if(selectCarBean == null){
			shoppingCarDAO.insertToCar(shoppingCarBean);
		}else{
			selectCarBean.setBookQuantity(selectCarBean.getBookQuantity()+shoppingCarBean.getBookQuantity());
